...
bottleneck (`docs/gemini.md` critique, citing LATTE). At the same time, direct agent-to-agent messaging
is expensive and error-prone (inter-agent misalignment is 32.3% of MAST failures). Stigmergy's practical
lesson for LLMs (`docs/grok.md`) is to coordinate through **shared state** (a blackboard), not chat.
...
- ~80% less coordination overhead than direct messaging (per `docs/grok.md`); great observability.
...
## Research basis
LATTE emergent task graphs (`docs/gemini.md`); stigmergy/blackboard synthesis (`docs/grok.md`);
Hearsay-II blackboard lineage and selective communication (STEAM) via `docs/claude.md`.
